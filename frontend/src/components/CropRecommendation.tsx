...
const CropRecommendation = () => {
  ...
  const fetchWeather = async (region: string) => {
    ...
    try {
      const res = await fetch(`http://127.0.0.1:8000/weather/${region}`);
...
const handlePredict = async () => {
  ...
  try {
    ...
    setResult({
      crop: data.recommended_crop.charAt(0).toUpperCase() + data.recommended_crop.slice(1),
      cropType: data.crop_type,
      tip: data.farming_tip,
      fertilizer: data.fertilizer,
      season: data.season,
      weatherAdvice: data.weather_advice,
      regionCrops: data.top_state_crops,
      aiCrops: data.top_ai_crops
    });
  } catch (error) {
